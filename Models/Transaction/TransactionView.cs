      ...
      var query = @"
        ...
        WITH Transactions_With_Changes AS (
          ...
          SELECT
            ...
            ,SUM(CASE WHEN LEN(check_number) > 0 THEN 1 ELSE 0 END) number_of_checks
        ...
        LEFT OUTER JOIN PaymentMethodSums PMS ON T.transaction_id = PMS.transaction_id
          ...
          WHERE child_transaction_id = @transaction_id
             OR T.transaction_id = @transaction_id
             OR (@transaction_id = -1 AND transaction_type = 'R')
